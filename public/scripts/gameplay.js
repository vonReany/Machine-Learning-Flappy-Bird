...
App.Main = function(game) {
    ...
    this.BARRIER_DISTANCE = gameHeight == 1080 ? 400 : 300;
}

App.Main.prototype = {
    preload: function() {
        this.game.load.spritesheet('imgBird', 'assets/img_bird_20.png', 36, 36, 40);
        if (gameHeight == 1080) {
            this.game.load.spritesheet('imgTree', 'assets/img_tree_1080.png', 120, 600, 2);
        } else {
    ...
    create: function() {
        ...
        for (var i = 0; i < this.GA.max_units; i++) {
            ...
            this.txtStatusCurr.push(new Text(this.game, posChangeW + 1270, y, "", "right", "fnt_digits_blue"));
        }

        // create a text object displayed in the HUD footer to show info of the best unit ever born
        this.txtBestUnit = new Text(this.game, posChangeW + 395, posChangeH + 570, "", "center", "fnt_chars_black");

        // create buttons
        this.btnRestart = this.game.add.button(posChangeW + 560, posChangeH + 545, 'imgButtons', this.onRestartClick, this, 0, 0);
        this.btnPause = this.game.add.button(posChangeW + 720, posChangeH + 545, 'imgButtons', this.onPauseClick, this, 1, 1);

        // create game paused info
        if (gameHeight == 1080) {
            this.sprPause = this.game.add.sprite(820, 540, 'imgPause');
        } else {
        ...
        switch (this.state) {
            ...
            case this.STATE_START: // start/restart the game
                ...
                this.txtBestUnit.text =
                    "Best bird born in " + (this.GA.best_population) + ":" +
                ...
                this.BarrierGroup.forEach(function(barrier) {
                    barrier.restart(400 + barrier.index * this.BARRIER_DISTANCE);
                }, this);
            ...
            case this.STATE_PLAY: // play Flappy Bird game by using genetic algorithm AI
                ...
                this.BirdGroup.forEachAlive(function(bird) {
                    ...
                    if (bird.alive) {
                        ...
                        if (bird.y < 0 || bird.y > gameHeight - 110) this.onDeath(bird);
...
TreeGroup.prototype.restart = function(x) {
    if (gameHeight == 1080) {
        var treeSpace = 170;
        var rangeL = 230 - this.topTree.height;
        var rangeR = -70;
    } else {
        var treeSpace = 150;
        var rangeL = 130 - this.topTree.height;
        var rangeR = -20;
    }
    this.topTree.reset(0, 0);
    this.bottomTree.reset(0, this.topTree.height + treeSpace);

    this.x = x;
    this.y = this.game.rnd.integerInRange(rangeL, rangeR);

    this.setAll('body.velocity.x', -200 * gameSpeed);
};
...
TreeGroup.prototype.getGapY = function() {
    return this.bottomTree.world.y - 65 - (gameHeight == 1080 ? 20 : 0);
};
